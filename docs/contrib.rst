...
Contributing to ``dropbox-api-primer``
======================================
...
You can `file new issues <https://github.com/posita/dropbox-api-primer/issues>`__ as you find them.
...
If you're willing and able, consider `submitting a pull request <https://github.com/posita/dropbox-api-primer/pulls>`__ (PR) with a fix.
...
*   Provide tests where feasible and appropriate.
    ...
    To set up a virtual environment (via ``virtualenv``) for development and to run tests inside that virtual environment, you can do the following::

        ( . ./venvsetup.sh && ./runtests.sh [-e TOX_ENV] )
